...
class UserAccountManagementControllerTest {
    ...
    void given_when_then() throws Exception {
        // Given
        given(userAccountManagementService.getUserAccountes()).willReturn(List.of());
        ...
        then(userAccountManagementService).should().getUserAccountes();
    }

    @WithMockUser(username = "tester", roles = "USER")
    @DisplayName("[data][GET] 회원 1개 - 정상 호출")
    @Test
    void givenUserAccountId_whenRequestUserAccount_thenReturnUserAccount() throws Exception {
        // Given
        String userId = "uno";
        ...
        // When  // Then
        mvc.perform(
                get("/management/user-accounts/" + userId)
                )
                .andExpect(status().isOk())
